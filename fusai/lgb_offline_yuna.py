...
if __name__ == "__main__":
    # 路径
    train_dir = '~/pro/ML/yuna/ogeek/data/data_train.txt'
    vali_dir = '~/pro/ML/yuna/ogeek/data/data_vali.txt'
    test_dir = '~/pro/ML/yuna/ogeek/data/data_test.txt'
    vec_dir_1 = '~/pro/ML/yuna/ogeek/data/w2v_model/w2v_total_50wei.model'
    ext_vec_dirs = [
        # '../data/merge_sgns_bigram_char300/merge_sgns_bigram_char300.txt',
        ]
    srop_word_dir = '~/pro/ML/yuna/ogeek/data/stop_words.txt'
    dictionary_dir = './dictionary.txt'
    test_result_dir = './lake_20181122.csv'

    # 导入数据
    train_fea_dir = '../../../../yuna/ogeek/test/train_weight_stat.csv'
    vali_fea_dir = '../../../../yuna/ogeek/test/vali_weight_stat.csv'
    print("-- 导入原始数据", end='')
    print(os.path.isfile(train_fea_dir), os.path.isfile(vali_fea_dir))
    ...
    # if False:
        ...
        # raw_train.drop(dropCols, axis=1).to_csv('./temp/train_fea_%s' % saveTime.strftime("%Y%m%d%H%M"), index=False)
        # raw_vali.drop(dropCols, axis=1).to_csv('./temp/vali_fea_%s' % saveTime.strftime("%Y%m%d%H%M"), index=False)
        # raw_test.drop(dropCols, axis=1).to_csv('./temp/test_fea_%s' % saveTime.strftime("%Y%m%d%H%M"), index=False)
        del raw_test
        gc.collect()
